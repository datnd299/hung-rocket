...
use App\PaymentChannels\Drivers\Sepay\Channel;
...
class PaymentController extends Controller
{
    ...
    {
        $order = Order::query()->where('reference_id', $code)->firstOrFail();
    ...
    public function bankTransfer($code, $chanelId, Request $request)
    {
        $order = Order::query()->where('reference_id', $code)->firstOrFail();
        $paymentChanel = PaymentChannel::query()->where('id', $chanelId)->firstOrFail();
        $channel = new Channel($paymentChanel);
        return view('web.default.payment.bank-transfer', compact('order', 'channel'));
    }
    ...
    public function paymentVerify2(Request $request, $chanelId)
    {
        $paymentChannel = PaymentChannel::where('id', $chanelId)
            ->where('status', 'active')
            ->first();
        $channelManager = ChannelManager::makeChannel($paymentChannel);
<<<<<<< HEAD
